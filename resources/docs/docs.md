...
* Let `mount` know to start the database connection by `require`-ing issues-combined.core in some other namespace.
...
Request middleware functions are located under the `issues-combined.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `issues-combined.dev-middleware` namespace found in
